...
export function ImportReceiptErrorDialog({
	isOpen,
	setIsOpen,
	errors,
}: IImportReceiptErrorDialogProps) {
	function handleCloseDialog() {
		setIsOpen(false);
	}

	const totalErrors = errors.reduce(
		(acc, error) => acc + error.pages.length,
		0,
	);
		...
		<>
			<Dialog open={isOpen} onOpenChange={setIsOpen}>
				<DialogContent>
					...
					<div>
						<p>
							Algumas páginas não seguiram o padrão de estrutura do tipo de
							recibo selecionado. Por favor, verifique as páginas listadas
							abaixo.
						</p>
						<p>As paginas corretas foram importadas normalmente.</p>
